...
The Wildfly jar maven plugin is aimed to build a bootable jar for WildFly (version 20.0.0.Final and greater). 
...
To get the list of the arguments the bootable jar supports:

```
java -jar target/myapp-wildfly.jar --help
```

A WildFly bootable jar behave in a way that is similar to a WildFly server started from a filesystem installation but with some limitations:

* The server can't be re-started automatically during a shutdown.
* Management model changes (eg: using JBoss CLI) are not persisted. Once the server is killed, management updates are lost.

NB: When started, the bootable jar installs a WildFly server in the tmp directory. The bootable jar console traces contain the path to this transient installation. This 
...
contains a list of maven example projects that highlight various usages of the WildFly bootable jar. Build and run these projects
...
When building a bootable jar you have the ability to select the set of https://docs.wildfly.org/19.1/Admin_Guide.html#defined-galleon-layers[WildFly server features] you want to see present in the bootable jar. Selecting a subset of server 
features has an impact on the server xml configuration and the set of installed JBoss modules. By selecting the subset required by your application you will reduce the
jar size, server configuration content and memory footprint.
...
If no Galleon layers are provided, a complete standalone server (with default _standalone.xml_ configuration) is provisioned and packaged in the bootable jar according to the WildFly feature-pack in use. 
...
In order to exclude layers that are not strictly required, use the link:#excludeLayers[exclude-layers] configuration element.
...
In addition to Galleon layers that you can use to configure the server according to your use-case, you can fine tune the server during packaging.

The maven plugin allows you to:

* Run JBoss CLI scripts (see https://github.com/wildfly-extras/wildfly-jar-maven-plugin/tree/master/examples/logging[logging] example to configure loggers).
...
The example https://github.com/wildfly-extras/wildfly-jar-maven-plugin/tree/master/examples/https[https] shows how to package a keystore file.
...
To enable slim bootable jar use the link:#pluginOptions[plugin-options] configuration element and add to it the _jboss-maven-dist_. For example:
...
The maven plugin can generate a maven repository directory containing all the JBoss modules artifacts required by the slim bootable jar.
...
_jboss-maven-repo_. For example:
...
and generate a local maven repository.
