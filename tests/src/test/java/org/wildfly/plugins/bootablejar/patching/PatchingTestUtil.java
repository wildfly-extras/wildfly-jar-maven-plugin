...
public class PatchingTestUtil {
    ...
    public static final String RELATIVE_MODULES_PATH = Joiner.on(FILE_SEPARATOR).join(new String[]{MODULES, SYSTEM, LAYERS, BASE});
    ...
    public static void buildMiscPatch(Path patchContentDir, boolean overrides, Path testDir, String patchID, String testContent, String... segments) throws Exception {
        Path targetFile = Paths.get(AS_DISTRIBUTION);
        for(String str : segments) {
            targetFile = targetFile.resolve(str);
        }
        final String originalContent = readFile(targetFile.toString());
        assertFalse(testContent.equals(originalContent));
        ContentModification miscFileModified = ContentModificationUtils.modifyMisc(patchContentDir.toFile(),
                patchID, testContent, targetFile.toFile(), segments);
        Patch oneOffPatch = PatchBuilder.create()
                .setPatchId(patchID)
                .setDescription("A one-off patch modifying a misc file.")
                .oneOffPatchIdentity(PRODUCT, AS_VERSION)
                .getParent()
                .addContentModification(miscFileModified)
                .build();
        generatePatchAndScript(oneOffPatch, patchContentDir, overrides, testDir);
    }

    public static void buildModulePatch(Path patchContentDir, boolean overrides, Path testDir, String patchID, Module newModule, String layerPatchID) throws Exception {
        ContentModification moduleAdded = ContentModificationUtils.addModule(patchContentDir.toFile(), layerPatchID, newModule);
        Patch oneOffPatch = PatchBuilder.create()
                .setPatchId(patchID)
    ...
    public static void buildModulePatch(Path patchContentDir, boolean overrides, Path testDir,
            String patchID, ContentModification moduleModified, String baseLayerPatchID) throws Exception {
        Patch oneOffPatch = PatchBuilder.create()
                .setPatchId(patchID)
    ...
    public static void buildCPPatch(Path patchContentDir, boolean overrides, Path testDir,
            String patchID, String cpAsVersion, ContentModification moduleModified,
            String baseLayerPatchID, ContentModification versionModuleModified, ContentModification cliModuleModified,
            ContentModification jbossModulesModification,
            ContentModification moduleA,
            ContentModification moduleB,
            ContentModification moduleC,
            ContentModification moduleD,
            ContentModification moduleE) throws Exception {
        Patch cpPatch = PatchBuilder.create()
                .setPatchId(patchID)
                .setDescription("A cp patch.")
                .upgradeIdentity(PRODUCT, AS_VERSION, cpAsVersion)
                .getParent()
                .upgradeElement(baseLayerPatchID, BASE, false)
                .addContentModification(moduleModified)
                .addContentModification(versionModuleModified)
                .addContentModification(cliModuleModified)
                .addContentModification(moduleA)
                .addContentModification(moduleB)
                .addContentModification(moduleC)
                .addContentModification(moduleD)
                .addContentModification(moduleE)
                .getParent()
                .addContentModification(jbossModulesModification)
                .build();
        generatePatchAndScript(cpPatch, patchContentDir, overrides, testDir);
    }

    private static void generatePatchAndScript(Patch patch, Path patchContentDir, boolean overrides, Path testDir) throws Exception {
        createPatchXMLFile(patchContentDir.toFile(), patch);
        File zippedPatch = createZippedPatchFile(patchContentDir.toFile(), "patch");
        Files.copy(zippedPatch.toPath(), testDir.resolve("patch.zip"));
        Files.write(testDir.resolve("patch.cli"), ("patch apply " + zippedPatch + (overrides ? " --override-all" : "") + "\npatch info --json-output").getBytes());
    }
    ...
    public static ContentModification updateModulesJar(final File installation, final File patchDir) throws IOException {
    ...
    public static ResourceItem createVersionItem(final String targetVersion) {
        final Asset newManifest = new Asset() {
            ...
            public InputStream openStream() {
                return new ByteArrayInputStream(createVersionString(targetVersion).getBytes(StandardCharsets.UTF_8));
            }
        };
    ...
    public static Module createVersionModule(final String targetVersion) {
        ...
        return new Module.Builder("org.jboss.as.version")
                .slot("main")
                .resourceRoot(item)
